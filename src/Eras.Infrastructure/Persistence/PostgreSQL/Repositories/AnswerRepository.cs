...
namespace Eras.Infrastructure.Persistence.PostgreSQL.Repositories
{
    ...
    public class AnswerRepository(AppDbContext context)
        : BaseRepository<Answer, AnswerEntity>(
            context,
            AnswerMapper.ToDomain,
            ...
            IAnswerRepository
    {
        public async Task<List<Answer>?> GetByStudentIdAsync(string uuid)
        {
            ...
            if (student == null)
                return null;
            var pollsOfStudent = await _context
                .PollInstances.Where(pollInstance => pollInstance.StudentId.Equals(student.Id))
                .ToListAsync();
            var lastPoll = pollsOfStudent
                .OrderByDescending(poll => poll.FinishedAt)
                .FirstOrDefault();
            ...
            var answers = await _context
                .Answers.Where(answer => answer.PollInstanceId.Equals(lastPoll.Id))
                .ToListAsync();
        ...
        {
            var answers = await _context
                .Answers.Where(answer => answer.PollInstanceId.Equals(uuid))
                .ToListAsync();
            var domainAnswers = new List<Answer>();
            foreach (var answer in answers)
            {
