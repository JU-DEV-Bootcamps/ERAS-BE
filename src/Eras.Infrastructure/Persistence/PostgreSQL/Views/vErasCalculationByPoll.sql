...
RiskCountByPollInstance AS (
    SELECT 
        a.poll_instance_id,
        s."name",
        SUM(a.risk_level) AS risk_count
    FROM 
        answers a
...
GROUP BY 
    poll_uuid, component_name, pc.poll_variable_id, question, pc.answer_text, a.poll_instance_id,rcbi."name", pc.answer_count, pc.percentage, rcbi.risk_count, rac.average_risk, rav.average_risk, c."name"
ORDER BY 
    pc.poll_variable_id, pc.percentage DESC;
